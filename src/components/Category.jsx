...
export default function Category() {
  ...
  const { category, categoryLoading, Recipes_Data } = useSelector(
    (state) => state
  );
  ...
  useEffect(() => {
    let separated = Recipes_Data.filter(
      (item) => item.data.recipe.category === category
    );
  ...
  if (categoryLoading) {
    return (
      <div style={{ position: "absolute", left: "40%", top: "35vh" }}>
        <MutatingDots
          height="100"
          width="100"
          color="grey"
