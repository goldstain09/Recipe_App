...
export default function AllRecipes() {
  ...
  const { Recipes_Data, searchValue, searchLoading, modal, dataKey } = useSelector((state) => state);
  ...
  useEffect(() => {
    if (searchValue) {
      let separating_according_Search = Recipes_Data.filter((item) => item.data.recipe.recipe_name.toLowerCase().includes(searchValue.toLowerCase()));
  ...
  // placeholderss  ----------
  const getting_data_loading = useSelector((state) => state.getting_data_loading);
  ...
  // loading
  if (searchLoading) {
    return (<div style={{ position: 'absolute', top: '30vh', left: '42%' }}><RotatingLines
      strokeColor="#46beff"
      strokeWidth="5"
    ...
    <>
      ...
      <div className='container allRecipes_Maindiv'>
        <div className='row d-flex'>
          {
            data_S.length > 0 ? data_S.map((item, index) => (
              <div key={item.id + index} className="card mb-3">
                <div className='row'>
                  <div className='col-sm-12 col-12 col-md-4 align-content-center justify-content-center'>
                    ...
                    <img alt='Recipe__Image' src={item.data.image_url} className="card-img-top" style={{ height: '20rem' }} />
                  </div>
